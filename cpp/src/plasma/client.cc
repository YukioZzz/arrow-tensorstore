...
Status PlasmaClient::Impl::GetBuffers(
  ...
  for (int64_t i = 0; i < num_objects; ++i) {
    ...
    if (object->data_size != -1) {
	  ...
	  ARROW_LOG(DEBUG)<<"Not found at the current process, creating object buffer based on its ipc handle";
	  ...
	  ARROW_LOG(DEBUG)<<"pysical_buf created";
        } else {
          iter->second->client_count++;
          physical_buf = MakeBufferFromGpuProcessHandle(iter->second);
        }
      ...
      ARROW_LOG(DEBUG)<<"All fields filled";
    } else {
